...
class DeviceResource(Resource):
    ...
    def _form_processing(self, request, is_active):
        ...
        if form.is_valid():
            form.save()
            p = Participant.objects.get(username=kwargs['data'].get('user_name'))
            if is_active:
                device_register = self.get_instance(**kwargs)
                for participant in Participant.objects.filter(device__dev_id = device_register.dev_id):
                    participant.device = None
                    participant.save()
                p.device = device_register
            else:
                p.device = None
            p.save()
